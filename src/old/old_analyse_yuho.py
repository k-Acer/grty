...
def make_lda_bow(texts, no_below, no_above, num_topics):
    ...
    if not os.path.isdir('./tmp/dic'):
        ...
        os.mkdir('./tmp/dic')
    d_fname = './tmp/dic/topics/topics-dic_{0}-{1}.txt'.format(no_below, int(no_above*100))
    ...
    # LDAモデルの作成
    lda = gensim.models.ldamodel.LdaModel(corpus=corpus, num_topics=num_topics, id2word=dictionary, alpha='auto', random_state=0)
    # ldaモデルの保存
...
def visualize_topics(lda, lda_param):
    ...
    # ワードクラウドの保存
    fname = './tmp/visualization/topics/topics-{0}_{1}-{2}-{3}.jpg'.format(lda_param[2], lda_param[0], lda_param[1], lda.num_topics)
    if not os.path.isfile(fname):
        print('save fig')
        plt.savefig(fname)
    
def evaluate_model(w_file, texts, limit, below, above, alpha):
    """
    モデルの評価を行う。
    ...
    for n_topic in tqdm(range(25, limit)):
        lda = gensim.models.ldamodel.LdaModel(corpus=corpus, id2word=dictionary, num_topics=n_topic, alpha=alpha, random_state=0)
...
def get_topics_2(lda, dictionary, text):
    """
    各有価証券報告書のトピック分布を推定する。
...
if __name__ == '__main__':
    ...
    # texts = load_texts('./data/topics_analysed/topics.pkl')
    # # BoW
    # lda, lda_param, corpus, dictionary = make_lda_bow(texts, 800, 0.2, 19)
    # hdp_boW
    ...
    ## トピックの取得
    firm_path = './data/topics/2020/1301'
    print(os.listdir(firm_path))
    ...
    #         for a in alpha:
    #             w_path = './tmp/evaluation/topics/{0}_{1}_{2}.png'.format(below, int(above*100), int(a*100))
    #             evaluate_model(w_path, texts, 27, below, above, a)
