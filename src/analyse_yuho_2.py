...
def sample(year):
    ...
    dictionary.filter_extremes(no_below=850, no_above=0.2)
    # 2006-2010 のテキストでコーパスを作成
    texts_2006_2010 = load_texts('./data/text_analysed/texts_2006_2010.pkl')
    corpus = [dictionary.doc2bow(text) for text in texts_2006_2010]
    # 学習
    lda = gensim.models.ldamodel.LdaModel(corpus=corpus, num_topics=15, id2word=dictionary, alpha=0.3, random_state=0)  
    # 2006-2010 のトピックを可視化
    visualize_topics(lda, 2010)
    # データフレーム用のリスト
    data = []
    # データの作成 2011-2020
    ...
    for y in year: 
        ...
        # １年ごとのデータを加えていく
        add_texts = load_texts('./data/text_analysed/texts_{0}.pkl'.format(y))
        other_corpus= [dictionary.doc2bow(text) for text in add_texts]
        lda.update(other_corpus)
        # データの可視化
        visualize_topics(lda, y)
        # データの作成
        target_dir = './data'
        data = make_data(lda, dictionary, target_dir, data, y)
    ...
    # csv で保存
    df.to_csv('risk_topics-bow_850_20_15_03.csv')

### make_data で加えられるデータは１年分
def make_data(lda, dictionary, target_dir, data, year):
    # データの作成
